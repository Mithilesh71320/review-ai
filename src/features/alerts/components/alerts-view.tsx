...
import { AlertTriangle, Award, RefreshCw, Shield, TrendingDown } from "lucide-react";
import { ListPageSkeleton } from "@/components/dashboard-skeletons";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Card } from "@/components/ui/card";
import { Switch } from "@/components/ui/switch";
import { useAlertsWorkspace } from "@/features/alerts/hooks/use-alerts-workspace";
...
export function AlertsView() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            ...
            size="icon"
            disabled={
              syncReviewsMutation.isPending || !selectedBusiness || !hasPaidPlan
            }
      ...
      {!hasPaidPlan && (
        <PaidPlanBanner message="Paid subscription required to sync fresh review alerts from Google." />
      )}
            ...
            <div className="flex min-h-[500px] items-center justify-center">
              <div className="max-w-md space-y-4 text-center">
                ...
                <p className="text-sm text-[#78716C]">
                  No new alerts. We&apos;ll notify you the moment something needs attention.
                </p>
            ...
            <div className="space-y-3">
              {data.alerts.map((alert) => {
                ...
                return (
                  <Card
                    key={alert.id}
                    className={`cursor-pointer rounded-xl border-[#E7E5E4] p-5 shadow-sm transition-colors hover:bg-[#FAFAF9] ${
                      !alert.read ? "bg-[#FEF3C7]/20" : "bg-white"
                    }`}
                  >
                    <div className="flex gap-4">
                      ...
                      <div className="flex flex-1 gap-4">
                        ...
                        <div className="flex-1 space-y-1">
                          ...
                          <Badge
                            variant="secondary"
                            className="mt-2 border-0 bg-[#F8F6F1] text-xs text-[#78716C]"
                          >
          ...
          <Card className="rounded-xl border-[#E7E5E4] bg-white p-6 shadow-sm">
            ...
            <div className="space-y-4">
              {data.rules.map((rule) => (
                <div
                  key={rule.id}
                  className="flex items-center justify-between border-b border-[#E7E5E4] pb-4 last:border-0 last:pb-0"
                >
                  <div>
